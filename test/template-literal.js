...
test('correctly perform template parsing', t => {
  const ctx = m.constructor({ level: 0 });
  t.is(
    ctx`{bold Hello, {cyan World!} This is a} test. {green Woo!}`,
    ctx.bold('Hello,', ctx.cyan('World!'), 'This is a') +
      ' test. ' +
      ctx.green('Woo!')
  );
});
...
test('correctly perform template substitutions', t => {
  ...
  t.is(
    ...
    ctx`{bold Hello, {cyan.inverse ${name}!} This is a} test. {green ${exclamation}!}`,
    ctx.bold('Hello,', ctx.cyan.inverse(name + '!'), 'This is a') +
      ' test. ' +
...
test('correctly parse and evaluate color-convert functions', t => {
  ...
  t.is(
    ctx`{bold.rgb(144,10,178).inverse Hello, {~inverse there!}}`,
  ...
  t.is(
    ctx`{bold.bgRgb(144,10,178).inverse Hello, {~inverse there!}}`,
...
test('throw if there is an unclosed block', t => {
  ...
  try {
    ...
    t.is(
      err.message,
      'Chalk template literal is missing 1 closing bracket (`}`)'
    );
  }

  try {
    console.log(
      ctx`{bold this shouldn't {inverse appear {underline ever\} :) \}`
    );
    t.fail();
  } catch (err) {
    t.is(
      err.message,
      'Chalk template literal is missing 3 closing brackets (`}`)'
    );
  }
});
...
test('allow custom colors (themes) on custom contexts', t => {
  ...
  t.is(
    ctx`Hello, {rose Rose}.`,
    'Hello, \u001B[38;2;246;217;217mRose\u001B[39m.'
  );
});
...
test('correctly parses unicode/hex escapes', t => {
  ...
  t.is(
    // eslint-disable-next-line max-len
    ctx`\u0078ylophones are fo\u0078y! {magenta.inverse \u0078ylophones are fo\u0078y!}`,
...
test('correctly parses string arguments', t => {
  const ctx = m.constructor({ level: 3 });
  t.is(
    ctx`{keyword('black').bold can haz cheezburger}`,
    '\u001B[38;2;0;0;0m\u001B[1mcan haz cheezburger\u001B[22m\u001B[39m'
  );
  t.is(
    ctx`{keyword('blac\u006B').bold can haz cheezburger}`,
    '\u001B[38;2;0;0;0m\u001B[1mcan haz cheezburger\u001B[22m\u001B[39m'
...
test('throws if a bad argument is encountered', t => {
  // Keep level at least 1 in case we optimize for disabled chalk instances
  const ctx = m.constructor({ level: 3 });
  try {
    ...
    t.is(
      err.message,
      "Invalid Chalk template style argument: ???? (in style 'keyword')"
    );
  }
});
