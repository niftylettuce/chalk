...
test('support applying multiple styles at once', t => {
  t.is(
    m.red.bgGreen.underline('foo'),
    '\u001B[31m\u001B[42m\u001B[4mfoo\u001B[24m\u001B[49m\u001B[39m'
  );
  t.is(
    m.underline.red.bgGreen('foo'),
...
test('reset all styles with `.reset()`', t => {
  t.is(
    m.reset(m.red.bgGreen.underline('foo') + 'foo'),
...
test('keep Function.prototype methods', t => {
  ...
  t.is(
    m.reset(m.red.bgGreen.underline.bind(null)('foo') + 'foo'),
    // eslint-disable-next-line max-len
    '\u001B[0m\u001B[31m\u001B[42m\u001B[4mfoo\u001B[24m\u001B[49m\u001B[39mfoo\u001B[0m'
  );
...
test('properly convert RGB to 16 colors on basic color terminals', t => {
  t.is(
    new m.constructor({ level: 1 }).hex('#FF0000')('hello'),
    '\u001B[91mhello\u001B[39m'
  );
  t.is(
    new m.constructor({ level: 1 }).bgHex('#FF0000')('hello'),
    '\u001B[101mhello\u001B[49m'
  );
});
...
test('properly convert RGB to 256 colors on basic color terminals', t => {
  t.is(
    new m.constructor({ level: 2 }).hex('#FF0000')('hello'),
    '\u001B[38;5;196mhello\u001B[39m'
  );
  t.is(
    new m.constructor({ level: 2 }).bgHex('#FF0000')('hello'),
    '\u001B[48;5;196mhello\u001B[49m'
  );
  t.is(
    new m.constructor({ level: 3 }).bgHex('#FF0000')('hello'),
